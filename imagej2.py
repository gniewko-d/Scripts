...
red_channel_rescale_original[red_channel_rescale_original<t1] = 0
red_channel_rescale_original[red_channel_rescale_original>0] = 1
method_chosen = "threshold_triangle"
ax[0,1].imshow(red_channel_rescale_original, cmap ="gray")
...
ax[1,1].hist(red_channel_rescale_original.flatten(), bins =10)
ax[1,1].set_title("Binarised data")


#segmentation with skimage and comercial filters
df = pd.DataFrame(columns= ["X", "Y", "X1", "Y1", "X2", "Y2"])
Red_mask = remove_small_objects(red_channel_rescale_original.astype(np.bool), min_size = 20)
...
ax[2].imshow(red_channel_rescale_original, cmap ="gray")
